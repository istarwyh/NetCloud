...
public class DownloadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {	
		...
		System.out.println("the downloadName is " + downloadName);
		...
		//对文件名进行处理，解决特殊字符
		String newdownloadName = downloadName.replace(" ", "%20").replace("#", "%23").
		...
		if(dirs.size() == 0) {
			...
			fs = FileSystem.get(URI.create(dealFile.getRoot() + "/usr/" + user.getUserdir() + "/" + newdownloadName), conf);
		} else {
			hadoopPath = new Path(dirs.get(dirs.size()-1).getFilepath() + "/" + downloadName);
			fs = FileSystem.get(URI.create(dirs.get(dirs.size()-1).getFilepath() + "/" + newdownloadName), conf);
		}
        ...
        fis.close();*/
        aes = new AES();
System.out.println("解密的 key is " + user.getKey() + "  iv is " + user.getIv());	        
